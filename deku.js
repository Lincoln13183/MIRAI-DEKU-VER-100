...
exec("rm -rf script/commands/data && mkdir -p script/commands/data && rm -rf script/commands/tad/* ", (error, stdout, stderr) => {
    ...
    console.log(chalk.bold.hex("#00FF00")("《 DEKU AUTO CLEAR CACHE 》 ❯ ") + chalk.hex("#00FF00")("Successfully delete cache"))
});
...
function checkBan(checkban) {
    ...
    logger(global.getText('mirai', 'checkListGban'), '《 DEKU GLOBAL BAN 》'), global.checkBan = !![];
    if (existsSync('/home/runner/.miraigban')) {
        ...
        logger(global.getText('mirai', 'banDevice'), '《 DEKU GLOBAL BAN 》'), _0x2cd8f4.on(line, _0x4244d8 => {
            ...
            if (isNaN(_0x4244d8) || _0x4244d8.length < 6 || _0x4244d8.length > 6) 
                ...
                // if (_0x2f978e.headers.server != 'cloudflare') return logger('BYPASS DETECTED!!!', '《 DEKU GLOBAL BAN 》'), 
                //  process.exit(0);
                ...
                if (_0x360aa8 !== _0x4244d8) return console.log(global.getText('mirai', 'codeInputExpired'));
                    ...
                    logger(global.getText('mirai', 'unbanDeviceSuccess'), '《 DEKU GLOBAL BAN 》');
                }
            });
        });
        return;
    ...
    return axios.get('https://raw.githubusercontent.com/siegfriedsama/Siegfried-Sama.-./main/data.json').then(dataGban => {
        ...
        //  return logger('BYPASS DETECTED!!!', '《 DEKU GLOBAL BAN 》'), 
        // process.exit(0);
        ...
        for (const adminID of admin) {
            if (!isNaN(adminID) && dataGban.data.hasOwnProperty(adminID)) {
                logger(global.getText('mirai','userBanned', dataGban.data[adminID]['dateAdded'], dataGban.data[adminID]['reason']), '《 DEKU GLOBAL BAN 》'), 
        ...
        if (dataGban.data.hasOwnProperty(checkban.getCurrentUserID())) {
            logger(global.getText('mirai', 'userBanned', dataGban.data[checkban.getCurrentUserID()]['dateAdded'], dataGban['data'][checkban['getCurrentUserID']()]['reason']), '《 DEKU GLOBAL BAN 》'), 
            mkdirSync(_0x4e5718 + ('/.miraigban'));
            if (_0x28e5ae == 'win32') 
        ...
        return axios.get('https://raw.githubusercontent.com/siegfriedsama/Siegfried-Sama.-./main/data.json').then(json => {
            ...
            //  return logger('BYPASS DETECTED!!!', '《 DEKU GLOBAL BAN 》'), 
            // process.exit(0);
            logger(json.data[Math['floor'](Math['random']() * json.data.length)], '《 DEKU BROAD CAST 》');
        }), logger(global.getText('mirai','finishCheckListGban'), '《 DEKU GLOBAL BAN 》');
    }).catch(error => {
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        if (!global.checkBan) logger(global.getText('mirai', 'warningSourceCode'), '《 DEKU GLOBAL BAN 》');
        global.client.api = loginApiData
        logger(`Franics`, '《 FRANCIS 》');
        logger('Hey, thank you for using this Bot', '《 FRANCIS 》');
        logger("Fixed by Francis", '《 FRANCIS 》');
        logger("FB: facebook.com/Francis.loydRaval", 'Fixed by Francis');
      var cron = require("node-cron");
...
loginApiData.sendMessage(`╭┉┉┅┉┅┄┄•◦ೋ•◦❥•◦ೋ
  ⟬🅕︎🅐︎🅑︎⟭ 𝗣𝗥𝗢𝗝𝗘𝗖𝗧 𝗗𝗘𝗞𝗨
•◦ೋ•◦❥•◦ೋ•┈┄┄┅┉┅┉╯\nฅ^•ﻌ•^ฅ== ☾︎ᴅᴇᴋᴜ☽︎ ==ฅ^•ﻌ•^ฅ\n\n     𝘽𝙊𝙏 𝙎𝙏𝘼𝙏𝙐𝙎 𝙐𝙋𝘿𝘼𝙏𝙀\n\n🙋‍♂️ | 𝖬𝖺𝗌𝗍𝖾𝗋, 𝖣𝖤𝖪𝖴 𝖠𝖨 𝖧𝖺𝗌 𝖡𝖾𝖾𝗇 𝖲𝗎𝖼𝖼𝖾𝗌𝗌𝖿𝗎𝗅𝗅𝗒 𝖠𝖼𝗍𝗂𝗏𝖺𝗍𝖾𝖽 𝖠𝗍 𝖳𝗁𝖾 𝗍𝗁𝖾 𝖮𝖿: ${time}\n\n[ 𝗡𝗢𝗧𝗘 ]  𝖠𝗎𝗍𝗈 𝖱𝖾𝗌𝗍𝖺𝗋𝗍 𝖤𝗏𝖾𝗋𝗒 30 𝖬𝗂𝗇𝗎𝗍𝖾𝗌 𝖳𝗈 𝖠𝗏𝗈𝗂𝖽 𝖦𝖾𝗍𝗍𝗂𝗇𝗀 𝖮𝖿𝖿\n\nฅ^•ﻌ•^ฅ== ☾︎ᴅᴇᴋᴜ☽︎ ==ฅ^•ﻌ•^ฅ`, global.config.ADMINBOT[0])
...
cron.schedule(`0 */30 * * * *`, () => {
  for (let pep of tet)
  loginApiData.sendMessage("╭┉┉┅┉┅┄┄•◦ೋ•◦❥•◦ೋ\n⟬🅕︎🅐︎🅑︎⟭ 𝗣𝗥𝗢𝗝𝗘𝗖𝗧 𝗗𝗘𝗞𝗨\n•◦ೋ•◦❥•◦ೋ•┈┄┄┅┉┅┉╯\n𝖬𝖺𝗌𝗍𝖾𝗋, 𝖺𝗎𝗍𝗈 𝗋𝖾𝗌𝗍𝖺𝗋𝗍 𝖾𝗏𝖾𝗋𝗒 30 𝗆𝗂𝗇𝗎𝗍𝖾𝗌 𝗍𝗈 𝖺𝗏𝗈𝗂𝖽 𝗀𝖾𝗍𝗍𝗂𝗇𝗀 𝗈𝖿𝖿.", pep,() => process.exit(1));
},{
  scheduled: true,
  timezone: "Asia/Manila"
});
...
cron.schedule('0 0 21 * * *', () => {
  ...
  timezone: "Asia/Manila"
        ...
        //         return logger(global.getText('mirai', 'refreshListen'), '《 DEKU DEV MODE 》');
        // }, 600000);
    });
}
//////////////////////////////////////////////
...
(async() => {
    try {
        ...
        logger(global.getText('mirai', 'successConnectDatabase'), '《 DEKU DATABASE 》');
        const botData = {};
        botData.models = models
        onBot(botData);
    } catch (error) { logger(global.getText('mirai', 'successConnectDatabase', JSON.stringify(error)), '《 DEKU DATABASE 》'); }
